...
package lv.tsi.javacourses.shipping.auth.boundary;

import lv.tsi.javacourses.shipping.auth.control.PasswordHashAlgorithm;
import lv.tsi.javacourses.shipping.auth.control.UserDAO;
import lv.tsi.javacourses.shipping.auth.model.Role;
...
public class RegistrationBean implements Serializable {
    ...
    public String register() {
        if (!Objects.equals(password1, password2)) {
            ...
            FacesContext.getCurrentInstance().addMessage("loginForm:pwdTxt", msg);
            return null;
        }
        try {
            ...
            FacesContext.getCurrentInstance().getExternalContext().getFlash().put("reg", "success");
            return "success";
        } catch (Exception e) {
            FacesMessage msg = new FacesMessage("Registration unsuccessful. Try another login name");
            FacesContext.getCurrentInstance().addMessage("loginForm:loginTxt", msg);
